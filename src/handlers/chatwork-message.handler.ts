...
export const handleChatMessage = async (event: any) => {
  try {
    ...
    // 6. Xử lý lệnh /reset
    if (cleanPrompt.trim() === '/reset') {
      conversationStore.resetHistory(roomId);
      const resetMsg = `[rp aid=${fromAccountId} to=${roomId}-${messageId}]\n✅ Đã xoá lịch sử hội thoại cho room này. Cuộc trò chuyện tiếp theo sẽ bắt đầu mới.`;
      await chatworkService.sendMessage(roomId, resetMsg);
      logger.info(`[CONVERSATION] room_id=${roomId} action=reset_via_command`);
      return;
    }

    // 7. Gọi Gemini AI (truyền roomId để duy trì conversation history)
    const aiResponse = await geminiService.generateResponse(roomId, cleanPrompt);

    // 8. Format câu trả lời trả về dạng Reply người dùng
    const finalResponse = `[rp aid=${fromAccountId} to=${roomId}-${messageId}]\n${aiResponse}`;

    // 9. Gửi lại lên Chatwork
